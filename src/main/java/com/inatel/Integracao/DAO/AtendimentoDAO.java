...
public class AtendimentoDAO extends ConnectionDAO {
    ...
    public boolean inserirAtendimento(Atendimento atendimento) {
        connectToDB();
        String sql = "INSERT INTO atendimento (situacao,data,hora,procedimento,Funcionarios_idFuncionarios,Paciente_idPaciente) values('aberto',?,?,?,?,?)";
        try {
    ...
    public boolean updateAtendimento(int id) {
        connectToDB();
        String sql = "UPDATE atendimento SET situacao = 'finalizado' WHERE idAtendimento = ?;";
        try {
            pst = con.prepareStatement(sql);
            pst.setInt(1, id);
            pst.execute();
    ...
    public ArrayList<Atendimento> pesquisarAtendimento(String nome) {
        ...
        connectToDB();
        String sql = "SELECT p.nome,a.situacao,a.data,a.hora,a.idAtendimento, a.procedimento, f.nome FROM atendimento AS a INNER JOIN paciente AS p INNER JOIN  funcionarios AS f ON a.Funcionarios_idFuncionarios = f.idFuncionarios AND p.nome = ?";
        try {
            pst = con.prepareStatement(sql);
            ...
            while (rs.next()) {
                ...
                atendimentos.setSituacao(rs.getString("a.situacao"));
